"""
ChatGPT Beer Analysis Integration - Label & Text Colors Only
-----------------------------------------------------------
...
BEERS_CSV = "beers.csv"
OUTPUT_CSV = "chatgpt_color_analysis.csv"
OUTPUT_JSON = "chatgpt_color_analysis.json"
...
Focus ONLY on color analysis. Be specific about what the colors suggest about the beer's character, style, and target audience.
"""
    return prompt

def analyze_colors_with_chatgpt(beer_data: Dict) -> Dict:
    """Analyze label and text colors using ChatGPT."""
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a beer label design expert specializing in color psychology and visual branding. Provide detailed color analysis in the requested JSON format."
                },
                {
                    "role": "user",
                    "content": prompt
        ...
        try:
            ...
            print(f"JSON parsing error for {beer_data['beer_name']}: {e}")
            print(f"Raw response: {content}")
            return {"error": "Failed to parse ChatGPT response"}
            
    except Exception as e:
        print(f"API error for {beer_data['beer_name']}: {e}")
...
def main():
    ...
    print("🎨 Starting ChatGPT Color Analysis...")
    ...
    for idx, row in merged_df.iterrows():
        ...
        analysis = analyze_colors_with_chatgpt(beer_data)
